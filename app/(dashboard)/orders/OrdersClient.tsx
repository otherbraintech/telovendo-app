...
export default function OrdersClient() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* CREATION MODAL */}
      {isCreating && (
        <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-xl p-4 md:p-8 flex items-start md:items-center justify-center overflow-y-auto" onClick={(e) => { if (e.target === e.currentTarget) setIsCreating(false) }}>
          <div className="relative w-full max-w-5xl bg-card border border-white/5 shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-in fade-in zoom-in-95 duration-500 flex flex-col md:flex-row min-h-[500px]">
             <button onClick={() => setIsCreating(false)} className="absolute -top-3 -right-3 md:-top-6 md:-right-6 size-10 md:size-14 bg-blue-600 text-white flex items-center justify-center hover:bg-blue-500 transition-all z-50 shadow-2xl border border-white/10 group active:scale-90 cursor-pointer"><X className="size-5 md:size-7 group-hover:rotate-90 transition-transform duration-300" /></button>
      ...
      {selectedOrder && !isCreating && (
        <div 
          className="fixed inset-0 z-40 bg-black/70 backdrop-blur-xl p-4 md:p-8 flex items-start md:items-center justify-center overflow-y-auto" 
          onClick={(e) => { if (e.target === e.currentTarget) { setSelectedOrder(null); setIsEditing(false); } }}
          onTouchStart={handleTouchStart}
          onTouchEnd={handleTouchEnd}
        >
          {/* NAVIGATION BUTTONS (DESKTOP) */}
          <div className="hidden md:flex fixed inset-x-4 top-1/2 -translate-y-1/2 justify-between items-center z-50 pointer-events-none">
          ...
          <div className="relative w-full max-w-5xl bg-card border border-white/5 shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-in fade-in zoom-in-95 duration-500 flex flex-col md:flex-row min-h-[500px]">
             <button onClick={() => { setSelectedOrder(null); setIsEditing(false); }} className="absolute -top-3 -right-3 md:-top-6 md:-right-6 size-10 md:size-14 bg-blue-600 text-white flex items-center justify-center hover:bg-blue-500 transition-all z-50 shadow-2xl border border-white/10 group active:scale-90 cursor-pointer"><X className="size-5 md:size-7 group-hover:rotate-90 transition-transform duration-300" /></button>
             <div className="flex-1 flex flex-col md:flex-row">
               <div className="w-full md:w-[48%] bg-muted/20 flex flex-col border-b md:border-b-0 md:border-r border-border p-6 shrink-0 md:sticky md:top-0">
                  <div className="relative aspect-square w-full bg-black/20 flex items-center justify-center overflow-hidden border border-border shadow-inner">
